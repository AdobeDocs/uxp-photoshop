...
While recording an Action via the Photoshop Actions panel, a UXP plugin can add a step by using [`require('photoshop').action.recordAction`](../../ps-reference/media/photoshopaction.md#recordaction).  The recorded step will include a function name and an object holding provided data.  In this way, the Action step will execute the function with the data when played.
...
```
The `executionContext` argument conforms to the argument used by [executeAsModal](../../ps-reference/media/executeasmodal.md).  There is no usage of the context related to Action recording at this time.
...
```
![recordAction](./../../ps-reference/media/assets/record-action.png)
